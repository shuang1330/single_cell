# This scripts runs enrichment analysis of significant eQTLs results. It consists  of three functions and requires all cell-type specific eQTL results 'coeqtls_fullresults_fixed.all.tsv.gz' (with all genes tested) and significant results 'coeqtls_fullresults_fixed.sig.tsv.gz'. To run it 
...
# enrich_small function run enrichment analysis of test_genes with background set of genes, store it to the table and also as a dotplot to output pdf file.
...
# enrichment_function load the input files and apply previously described functions to selected genes.
